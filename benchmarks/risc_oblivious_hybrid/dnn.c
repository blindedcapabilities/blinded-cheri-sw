...
static int seed = 100;
static int nInputs = 128;
static int nL1Neurons = 512;
static int nOutputs = 8;
...
void __attribute__((noinline)) Inference(float* input, float* output, float* W1, float* B1, float* W2, float* B2){
    ...
    for(int i = 0; i < nOutputs; i++){
        ...
        /// Compute SoftMax(L2Output) with numerical stability fix.
    // Find the maximum value of L2Output to subtract from each element.
    float max_val = L2Output[0];
    for (int i = 1; i < nOutputs; i++) {
        if (L2Output[i] > max_val)
            max_val = L2Output[i];
    }
    ...
    float sum = 0.0;
    for (int i = 0; i < nOutputs; i++) {
        // Subtract max_val to avoid overflow in exp.
        L2Output[i] = exp(L2Output[i] - max_val);
        sum += L2Output[i];
    }
