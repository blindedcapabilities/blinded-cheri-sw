...
static int seed = 0;
static int nInputs = 1 << 6;
static int nL1Neurons = 1 << 12;
...
void __attribute__((noinline)) Inference(float* input, float* output, float* W1, float* B1, float* W2, float* B2){
    float* L1Output = (float*) blinded_malloc(sizeof(float) * nL1Neurons);
    float* L2Output = (float*) blinded_malloc(sizeof(float) * nOutputs);
...
int main(){
    ...
    // Initialize inputs
    float* input = (float*) blinded_malloc(sizeof(float) * nInputs);
    for(int i = 0; i < nInputs; i++)
        input[i] = GenRandFloat(3);


    // Initialize outputs
    float* output = (float*) blinded_malloc(sizeof(float) * nOutputs);
